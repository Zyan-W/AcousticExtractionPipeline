...
- Startup scripts run a quiet NumPy/PyTorch/MFA/tokenizer runtime check before downloading MFA models; if an existing environment is stale or missing MFA Japanese tokenizer dependencies (`spacy`, `sudachipy`, `sudachidict-core`), Korean tokenizer dependencies (`python-mecab-ko`, `jamo`), or Mandarin Chinese tokenizer dependencies (`spacy-pkuseg`, `dragonmapper`, `hanziconv`), they run `env update -n auto-mfa -f environment.yml --prune` before launching.
...
- `environment.yml` defines an isolated `auto-mfa` mamba environment for `python=3.11`, `numpy<2`, `ffmpeg`, `montreal-forced-aligner`, `spacy`, `sudachipy`, `sudachidict-core`, `python-mecab-ko`, `jamo`, `spacy-pkuseg`, `dragonmapper`, `hanziconv`, and `openai-whisper`.
- The main tool assumes `whisper`, `ffmpeg`, and `mfa` are available in the active environment or through `mamba run -n auto-mfa`.
- The main GUI includes a `Check Environment` button that verifies the active Python, `whisper`, `ffmpeg`, `mfa`, the NumPy/PyTorch bridge, and Japanese/Korean/Chinese tokenizer support before a long Whisper/MFA run.
...
- Offline release build tooling should create/update the `auto-mfa` environment, download the GUI's official MFA presets, download Whisper `small`, collect third-party license files into `licenses/`, pack the Windows environment with `conda-pack`, and emit a manifest plus SHA-256 checksums.
- The generated Windows offline bundle should preserve `licenses/THIRD_PARTY_LICENSES.md` and `licenses/third_party_licenses_manifest.json` so offline recipients can inspect the bundled conda/pip package license evidence without internet access.
